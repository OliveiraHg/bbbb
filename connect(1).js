    ...
    if (isWelcome) {
        if (participants.length === 1) {
            profilePicUrl = await NazunaSock.profilePictureUrl(participants[0], 'image').catch(() => profilePicUrl);
             ...
             profilePicUrl = await NazunaSock.profilePictureUrl(groupMetadata.id, 'image').catch(() => profilePicUrl);
        }

        const adReplyInfo = {
            title: `🌙🌹𝙀𝙄𝙏𝘼, ${participants.length > 1 ? '𝙉𝙊𝙑𝙊𝙎 𝙈𝙀𝙈𝘽𝙍𝙊𝙎' : '𝙉𝙊𝙑𝙊 𝙈𝙀𝙈𝘽𝙍𝙊'}🌹😈`,
            body: `𝘽𝙚𝙢-𝙫𝙞𝙣𝙙𝙤(𝙖/𝙨) 𝙖𝙤 ${groupMetadata.subject}`,
            thumbnailUrl: profilePicUrl,
            mediaType: 1,
            renderLargerThumbnail: true,
            showAdAttribution: false,
            sourceUrl: '' // Adicione um link se desejar que a prévia tenha um link
        };
        
        message.contextInfo = {
            externalAdReply: adReplyInfo,
            mentionedJid: mentions 
        };
    }
    
    if (settings.image) {
        let currentProfilePicUrl = 'https://raw.githubusercontent.com/nazuninha/uploads/main/outros/1747053564257_bzswae.bin';
        if (participants.length === 1 && isWelcome) {
            currentProfilePicUrl = await NazunaSock.profilePictureUrl(participants[0], 'image').catch(() => currentProfilePicUrl);
        }
...
async function handleGroupParticipantsUpdate(NazunaSock, inf) {
    try {
        ...
        switch (inf.action) {
            case 'add': 
                // Função de boas-vindas e blacklist (removido o X9 daqui)
                const membersToWelcome = [];
                const membersToRemove = [];
                for (const participant of inf.participants) {
                    if (globalBlacklist[participant] || groupSettings.blacklist?.[participant]) {
                        membersToRemove.push(participant);
                        continue;
                    }
                    if (groupSettings.bemvindo) membersToWelcome.push(participant);
                }
                ...
                if (membersToWelcome.length > 0) {
                    ...
                    await NazunaSock.sendMessage(from, message).catch(() => {});
                }
                break;

            case 'remove':
                // Apenas mensagem de saída (removido o X9 daqui)
            ...
            case 'promote':
            case 'demote': {
                const AUTHOR = inf.author;
                const TARGET = inf.participants[0];
                
                const BOT_PHONE = '5521977010904';
                const BOT_LID = '95589530382490';

                const cleanBaseJid = (jid) => {
                    if (!jid) return '';
                    let base = String(jid).split('@')[0];
                    if (base.includes(':')) {
                        base = base.split(':').pop();
                    }
                    return base.replace('+', '').replace(/[^0-9]/g, ''); 
                };

                const autorLimpo = cleanBaseJid(AUTHOR);
                const alvoLimpo = cleanBaseJid(TARGET);
                const donoLimpo = cleanBaseJid(config.numerodono);
                const lidOwnerBase = config.lidowner ? cleanBaseJid(config.lidowner) : null;

                const isBotAuthor = autorLimpo === BOT_PHONE || autorLimpo === BOT_LID;
                const isBotTarget = alvoLimpo === BOT_PHONE || alvoLimpo === BOT_LID;

                if (isBotAuthor || isBotTarget || !AUTHOR) {
                    return;
                }

                const isAuthorOwner = autorLimpo === donoLimpo || (lidOwnerBase && autorLimpo === lidOwnerBase);
                const isTargetOwner = alvoLimpo === donoLimpo || (lidOwnerBase && alvoLimpo === lidOwnerBase);

                if (isAuthorOwner || isTargetOwner) {
                    return; 
                }

                if (groupSettings.x9) {
                    const hasPermission = groupSettings.adminPerms?.active && 
                                        groupSettings.adminPerms.permissions?.some(p => {
                                            const pLimpo = cleanBaseJid(p.id);
                                            return autorLimpo === pLimpo && p.can_demote;
                                        });

                    if (!hasPermission) {
                        const isPromote = inf.action === 'promote';
                        
                        await NazunaSock.groupParticipantsUpdate(from, [AUTHOR], 'demote').catch(() => {});
                        
                        if (isPromote) {
                            await NazunaSock.groupParticipantsUpdate(from, [TARGET], 'demote').catch(() => {});
                        }

                        const alertaTexto = isPromote 
                            ? `⚠️ *VIOLAÇÃO DE SEGURANÇA: PROMOÇÃO*\n\n` +
                              `👤 *Alvo da Ação:* @${alvoLimpo}\n` +
                              `👮‍♂️ *Infrator:* @${autorLimpo}\n\n` +
                              `🚨 *Ação:* O administrador tentou promover um membro sem permissão. Como medida de segurança, *ambos foram rebaixados* imediatamente.`
                            : `ℹ️ *ALERTA: REBAIXAMENTO INDEVIDO*\n\n` +
                              `👤 *Alvo da Ação:* @${alvoLimpo}\n` +
                              `👮‍♂️ *Infrator:* @${autorLimpo}\n\n` +
                              `🚨 *Ação:* Este administrador realizou um rebaixamento sem autorização e foi *removido do cargo* de forma automática por abuso de autoridade.`;
...
process.on('uncaughtException', async (error) => {
    ...
    // Não tente apenas limpar, feche para resetar o estado
    process.exit(1); 
});
